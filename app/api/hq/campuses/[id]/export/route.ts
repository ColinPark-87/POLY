...
// Returns a date string "YYYY-MM-DD" from a DB date value (handles both string and timestamp)
...
export async function GET(
  ...
  for (const emp of employees ?? []) {
    ...
    // Individual leave dates (col 48+): date row = "YYYY-MM-DD", type row = "연차"/"오전반차"/etc.
    leaves.forEach((lv, i) => {
