...
export default function LoginPage() {
  ...
  function buildAhk(num: number) {
    ...
    return [
      ...
      `Loop`,
      `{`,
      `  ; 추적 중인 창이 없으면: 숨겨진 것 포함 기존 창 찾고, 없으면 새로 실행`,
      `  if (!hwnd or !WinExist("ahk_id " hwnd))`,
      `  {`,
      `    existing := WinExist("${tag}")`,
      `    if (existing)`,
      `      hwnd := existing`,
      `    else`,
      `    {`,
      ...
      `  if InStr(t, "_ACTIVE")`,
      `  {`,
      `    ; 수업시간: 보이게+복원+최대화+활성화 (매 루프 강제 → 가려져도 다시 앞으로)`,
      `    WinShow, ahk_id %hwnd%`,
      ...
      `  {`,
      `    ; 대기시간: 완전히 숨김 (작업표시줄에서도 사라져 되살아나지 않음)`,
      `    WinHide, ahk_id %hwnd%`,
      `  }`,
      `  Sleep, 1500`,
      `}`,
