...
export default function LoginPage() {
  ...
  function buildAhk(num: number) {
    ...
    return [
      ...
      `    continue`,
      `  }`,
      `  ; IDLE인데 화면에 보이면(최소화 아님) 강제 최소화`,
      `  if (prevState = "IDLE" and WinExist("POLLY_ATTENDANCE_IDLE"))`,
      `  {`,
      ...
      `  {`,
      `    ; 상태가 막 ACTIVE로 바뀐 순간에만 1회 띄움 (깜빡임 방지)`,
      `    if (prevState != "ACTIVE")`,
      `    {`,
      `      WinActivate`,
      `      WinRestore`,
      ...
      `  {`,
      `    ; 상태가 막 IDLE로 바뀐 순간에만 1회 숨김`,
      `    if (prevState != "IDLE")`,
      `    {`,
      `      WinActivate, POLLY_ATTENDANCE_IDLE`,
      `      Send, {F11}`,
      `      Sleep, 300`,
      `      WinMinimize, POLLY_ATTENDANCE_IDLE`,
      `      prevState := "IDLE"`,
